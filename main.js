...
let filenames = ["data/genre.csv", "data/duration.csv"];
...
svg.append("text")
    .attr("transform", `translate(${(width - margin.right - margin.left) /2}, ${height-margin.bottom})`)       // HINT: Place this at the bottom middle edge of the graph
...
let y_axis_text = svg.append("text")
    .attr("transform", `translate(-130, ${(height - margin.top - margin.bottom) / 2})`)       // HINT: Place this at the center left edge of the graph
    .style("text-anchor", "middle");
...
function setData(index, attr) {
    ...
    d3.csv("../" + filenames[index]).then(function(data) {
        ...
        console.log(x.range())
        title.text("Top Artists in Billboard 100 Charts");
